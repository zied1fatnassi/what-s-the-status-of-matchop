"""Data normalisation and cleaning functions."""
...
TRACKING_QUERY_KEYS = {
    ...
    "ref", "referrer", "trk", "trackingid", "gh_jid",
}
...
_TITLE_NOISE_PATTERNS = [
    re.compile(r"\s*[-–—|/]\s*(m/f/d|m/w/d|h/f|f/m/d|all genders)\s*", re.IGNORECASE),
    re.compile(r"\s*\(m/f/d\)\s*", re.IGNORECASE),
    re.compile(r"\s*\(h/f\)\s*", re.IGNORECASE),
]


def normalize_title(title: str) -> str:
    """Clean common noise from job titles."""
    cleaned = title
    for pattern in _TITLE_NOISE_PATTERNS:
        cleaned = pattern.sub("", cleaned)
    return normalize_whitespace(cleaned) or title
...
_COMPANY_SUFFIXES = re.compile(
    r"\s*,?\s*(?:Inc\.?|LLC|Ltd\.?|Corp\.?|GmbH|S\.?A\.?|PLC|Pty\.?\s*Ltd\.?)\s*$",
    re.IGNORECASE,
)
...
def normalize_company(company: str | None) -> str | None:
    ...
    if not cleaned:
        return None
    # Strip common legal suffixes for consistency
...
def normalize_salary(salary: str | None) -> str | None:
    """Attempt to extract and standardise salary range."""
    if not salary:
        return None
...
def parse_posted_at(raw: str | datetime | None, reference_time: datetime) -> str | None:
    ...
    if relative_match:
        ...
        return to_iso8601(reference_time - delta_map.get(unit, timedelta()))
...
TAG_PATTERNS = (
    ...
    (re.compile(r"\bdevops\b|\bsite reliability\b|\bsre\b|\bplatform engineering\b|\binfrastructure\b", re.IGNORECASE), "DevOps"),
)
...
def normalize_tags(
    ...
    for tag in tags:
        normalised = normalize_whitespace(tag)
        if not normalised:
            continue
        marker = normalised.casefold()
        if marker in seen:
            continue
        seen.add(marker)
...
def normalize_job(raw_job: RawJobPosting, seen_at: datetime | None = None) -> NormalizedJobPosting:
    """Normalise a raw scraped job into a database-ready form."""
    ...
    company_name = normalize_company(raw_job.company_name)
    location = normalize_location(raw_job.location)
    description = strip_html(raw_job.description)
    salary_range = normalize_salary(raw_job.salary_range)
