...
serve(async (req) => {
  ...
  try {
    ...
    if (!consumeRateLimit(user.id)) {
      return jsonResponse({ code: 'RATE_LIMITED', message: 'Too many requests. Please retry in a minute.' }, 429)
    }
    ...
    if (!mode) {
      return jsonResponse({ code: 'BAD_REQUEST', message: 'mode must be "standard" or "premium"' }, 400)
    }
    ...
    try {
      ...
      return jsonResponse({ code: 'BAD_REQUEST', message: 'Invalid cursor' }, 400)
    }
    ...
    if (userProfilesRes.error) {
      if (!userProfilesRes.error.message?.toLowerCase().includes('user_profiles')) {
        return jsonResponse({ code: 'DB_ERROR', message: 'Failed to load user profile links' }, 500)
      }
      ...
      if (studentsOnly.length === 0) {
        return jsonResponse({ code: 'FORBIDDEN', message: 'Swipe stack is only available for student profiles' }, 403)
      }
    } else {
    ...
    return jsonResponse({ code: 'INTERNAL_ERROR', message: 'Failed to build swipe stack' }, 500)
  }
})
